...
#for( k in 1:14)  {tabella[k]<- Valori_ogni_5min(as.data.frame(p[k]))}
...
tabella_testdplyr <- tabella %>%
  select(DATE:USDJPY) %>%
  dplyr::mutate_at(vars(AUDJPY:USDJPY),
                   funs(FiveminBefore= Column_before_Xmin(.,5),
                        TenminBefore=Column_before_Xmin(.,10),
                        FifteenminBefore=Column_before_Xmin(.,15),
                        MA2= ma(.,2),
                        MA3=ma(.,3),
                        MA4=ma(.,4),
                        MA5=ma(.,5)
